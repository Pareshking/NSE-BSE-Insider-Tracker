...
def extract_tables(dr):
 return dr.execute_script("""return Array.from(document.querySelectorAll('table')).map(t=>({rows:Array.from(t.querySelectorAll('tr')).map(r=>Array.from(r.cells).map(c=>(c.innerText||'').trim())).filter(x=>x.length),links:Array.from(t.querySelectorAll('a')).map(a=>({text:(a.innerText||'').trim(),href:a.href,onclick:a.getAttribute('onclick')||''}))})).filter(x=>x.rows.length);""")

def controls(dr):
 return dr.execute_script("""return Array.from(document.querySelectorAll('input,select,button')).map(x=>({type:x.type||'',name:x.name||'',id:x.id||'',value:x.value||'',text:(x.innerText||'').trim()})).filter(x=>x.type==='date'||/date|from|to|search|go/i.test((x.name||'')+' '+(x.id||'')+' '+(x.text||'')));""")
...
def browser_tables(url, symbol=None):
 ...
 tabs=extract_tables(d); ins=controls(d); body=d.find_element('tag name','body').text; d.quit(); return tabs,ins,body
...
def bse(d):
 ...
 from selenium.webdriver.chrome.options import Options
 o=Options(); [o.add_argument(x) for x in ('--headless=new','--no-sandbox','--disable-dev-shm-usage','--disable-gpu',f'--user-agent={UA}')]
 try: dr=webdriver.Chrome(options=o)
 except Exception as e:return [('browser_init',[],{'method':'BSE Selenium','error':str(e)})]
 for ds,u in pages.items():
  try:
   dr.get(u); time.sleep(4); tabs=extract_tables(dr); ctl=controls(dr); rows=[]; links=[]
   for t in tabs: rows+=t['rows']; links+=t['links']
   hist={'attempted':False,'status':'not_available'}
   date_nodes=[c for c in ctl if c['type']=='date' or any(z in nt(str(c)) for z in ('FROM DATE','TO DATE','DATE FROM','DATE TO'))]
   if date_nodes:
    hist={'attempted':True,'status':'no_match','target_date':HIST.isoformat()}
    try:
     val=HIST.strftime('%Y-%m-%d')
     dr.execute_script("""const v=arguments[0];const ns=Array.from(document.querySelectorAll('input,select')).filter(x=>x.type==='date'||/date|from|to/i.test((x.name||'')+' '+(x.id||'')));for(const n of ns){try{n.value=v;n.dispatchEvent(new Event('input',{bubbles:true}));n.dispatchEvent(new Event('change',{bubbles:true}))}catch(e){}}const bs=Array.from(document.querySelectorAll('button,input[type=button],input[type=submit]')).filter(x=>/search|submit|go|apply|refresh/i.test((x.innerText||x.value||'').trim()));if(bs.length)bs[0].click();""",val); time.sleep(4)
     ht=extract_tables(dr); hr=[]
     for t in ht: hr+=t['rows']
     matched=sum(1 for r in hr if any(dt(v)==HIST for v in r))
     hist.update(status='success' if matched else 'no_match',matched_rows=matched,rows=len(hr),controls_used=date_nodes[:10])
    except Exception as e:hist.update(status='error',error=str(e))
   out.append((ds,rows,{'method':'BSE Selenium','controls':ctl,'links':links,'table_count':len(tabs),'rows':len(rows),'target_date_text':any(x in dr.find_element('tag name','body').text for x in (d.strftime('%d/%m/%Y'),d.strftime('%d %b %Y'),d.strftime('%d %b %y'))),'historical_date_test':hist}))
  except Exception as e: out.append((ds,[],{'method':'BSE Selenium','error':str(e)}))
 dr.quit(); return out

def nse_issue(ds,url,symbol):
 try:
  tabs,ctl,body=browser_tables(url,symbol); rows=[]
  for t in tabs: rows+=t['rows']
  return rows,{'method':'NSE Selenium XBRL','controls':ctl,'table_count':len(tabs),'symbol_probe':symbol}
...
def key(ds,x):
 if ds=='insider_trading':return(x.get('date_from') or x.get('acquisition_date'),x.get('date_to'),x['company_norm'],x['person_norm'],x['side_norm'],x['quantity_num'],num(x.get('value')))
 ...
 for i,a in enumerate(n):
  for j,z in enumerate(b):
   if ds=='insider_trading':
    company=SequenceMatcher(None,a['company_norm'],z['company_norm']).ratio() if a['company_norm'] and z['company_norm'] else 0;person=SequenceMatcher(None,a['person_norm'],z['person_norm']).ratio() if a['person_norm'] and z['person_norm'] else 0;q=a['quantity_num'] is not None and a['quantity_num']==z['quantity_num'];side=bool(a['side_norm'] and z['side_norm'] and a['side_norm'][0]==z['side_norm'][0]);score=.4*company+.3*person+.2*q+.1*side
   else:
    sec=(a['symbol_norm']==z['symbol_norm']) if a['symbol_norm'] and z['symbol_norm'] else SequenceMatcher(None,a['company_norm'],z['company_norm']).ratio()>.92;q=a['quantity_num'] is not None and a['quantity_num']==z['quantity_num'];p=a['price_num'] is not None and a['price_num']==z['price_num'];person=SequenceMatcher(None,a['person_norm'],z['person_norm']).ratio() if a['person_norm'] and z['person_norm'] else 0;score=.4*sec+.3*q+.2*p+.1*(person>.9)
...
def main():
 ...
 for d,label in ((TARGET,'target'),(HIST,'historical')):
  for ds,fn in [('insider_trading',nse_insider),('bulk_deals',lambda x:nse_deals('bulk_deals',x)),('block_deals',lambda x:nse_deals('block_deals',x))]:
   rows,meta=fn(d); raw.append({'source':'NSE','dataset':ds,'label':label,'date':d.isoformat(),'rows':nse_norm(ds,rows),'meta':meta})
 ...
 for it in raw:
  rows=[identity(x) for x in it['rows']];u,du=dedup(it['dataset'],rows) if rows else([],[]);report['source_specific'].append({'source':it['source'],'dataset':it['dataset'],'label':it['label'],'probe_date':it['date'],'raw_count':len(it['rows']),'unique_count':len(u),'duplicate_count':len(du),'meta':it['meta'],'sample':u[:5],'duplicates':du[:20]});canon[(it['source'],it['dataset'],it['label'])]=u
