"""NSE Bulk Deals acquisition wrapper with explicit historical date evidence."""
...
with NSE(download_folder=str(OUT),server=True,timeout=40) as nse:
    ...
    for start in [TARGET, TARGET-timedelta(days=6), TARGET-timedelta(days=29), TARGET-timedelta(days=LOOKBACK-1)]:
        ...
        (OUT/f'{name}.json').write_text(json.dumps({'dataset':'bulk_deals','source':'NSE','start_date':str(start),'end_date':str(TARGET),'count':len(batch),'distinct_dates':dates,'rows':batch},indent=2,default=str)); rows.extend(batch)
report={'dataset':'bulk_deals','source':'NSE','target_date':str(TARGET),'lookback_days':LOOKBACK,'windows':windows,'count':len(rows),'unique_observations':len({json.dumps(r,sort_keys=True,default=str) for r in rows}),'columns':sorted(rows[0]) if rows else [],'rows':rows}
