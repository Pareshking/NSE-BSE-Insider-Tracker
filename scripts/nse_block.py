...
def fetch_range(d, start, end, retries=3):
    """Single API call for one [start, end] sub-range. Returns (url, rows, chunk_diag)."""
...
def fetch_all(d, earliest, latest):
    ...
    most-recent-first as first assumed. Confirmed 2026-09-01 with CHUNK=7 on
    both Bulk and Block (same underlying endpoint): every 7-day chunk
    returned exactly 70 rows, ALL from that chunk's FIRST (oldest) day --
    e.g. requesting 26-Aug..31-Aug returned only 26-Aug's deals, silently
    dropping 27-31 Aug entirely. That meant the 1d window (which needs the
    single most RECENT date) came back empty even though the 90d window had
    hundreds of real rows spanning many dates -- all of them chunk-start
    dates, none of them the target date.
    ...
    scratch -- see nse_bulk.py's fetch_all() docstring for why (a run where
    every window re-fetched from scratch made ~20 heavily-overlapping calls,
    and Bulk failed outright in that run while Block, in a fresh session
    right after, succeeded -- fetching once and slicing cuts total calls
    roughly in half with zero redundant overlap)."""
