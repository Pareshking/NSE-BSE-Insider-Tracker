"""NSE Block Deals acquisition with direct historical API evidence."""
...
def fetch(s,start,end):
    u=f'{BASE}/api/historical/block-deals'; p={'from':start.strftime('%d-%m-%Y'),'to':end.strftime('%d-%m-%Y')}
...
def main():
    ...
    report={'dataset':'block_deals','source':'NSE','target_date':str(TARGET),'lookback_days':LOOKBACK,'method':'NSE first-party historical API','windows':windows,'count':len(all_rows),'unique_observations':len({json.dumps(r,sort_keys=True,default=str) for r in all_rows}),'columns':sorted(all_rows[0]) if all_rows else [],'rows':all_rows}
