...
def main():
 ...
 raw=json.loads(Path('artifacts/data_validation_v3/bse_raw.json').read_text())['datasets']
 for ds,obj in raw.items():
  rows=[]
  for t in obj['tables']:rows+=t['rows']
  b=[ident(norm_bse(ds,r)) for r in rows];u,du=dedup(ds,b);report['source_specific'].append({'source':'BSE','dataset':ds,'label':'target','date':str(T),'raw_count':len(rows),'unique_count':len(u),'duplicate_count':len(du),'controls':obj.get('controls',[]),'method':'BSE Selenium full-table capture'});canon[('BSE',ds)]=u;Path(OUT/f'bse_{ds}.json').write_text(json.dumps(u,indent=2,default=str))
 for ds in ('insider_trading','bulk_deals','block_deals'):
  n=canon.get(('NSE',ds),[]);b=canon.get(('BSE',ds),[]);report['cross_exchange'].append({'dataset':ds,'nse_unique':len(n),'bse_unique':len(b),'candidate_matches':match(n,b,ds),'rule':'insider candidates may represent the same disclosure; bulk/block candidates are flags only and are never automatically collapsed because execution venues are distinct'})
