"""NSE Preferential Issue extraction/validation with network/API diagnostics."""
...
TARGET=date.fromisoformat(os.getenv('TARGET_DATE','2026-08-31')); LOOKBACK=int(os.getenv('LOOKBACK_DAYS','90'))
URL='https://www.nseindia.com/companies-listing/corporate-filings-PREF'; OUT=Path('artifacts/nse_validation/preferential')
UA='Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/139 Safari/537.36'

def browser():
    o=Options()
    for x in ['--headless=new','--no-sandbox','--disable-dev-shm-usage','--disable-gpu','--window-size=1920,1080','--lang=en-US',f'--user-agent={UA}']:
        o.add_argument(x)
    ...
    for item in d.get_log('performance'):
        try:
            ...
            if msg.get('method')=='Network.requestWillBeSent':
                ...
                if 'nseindia.com' in u and ('/api/' in u or 'PREF' in u.upper()):
