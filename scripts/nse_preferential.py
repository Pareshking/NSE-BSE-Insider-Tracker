"""NSE Preferential Issue extraction using first-party corporate-filings APIs."""
...
APIS=['https://www.nseindia.com/api/corporate-further-issues-pref?index=FIPREFIP','https://www.nseindia.com/api/corporate-further-issues-pref?index=FIPREFLS']
...
def dedup(rows):
    ...
    return out
def main():
    d=browser(); report={'source':'NSE','dataset':'preferential_issue','url':URL,'target_date':str(TARGET),'lookback_days':LOOKBACK,'api_endpoints':APIS,'windows':[]}
    widest_rows=[]
    try:
