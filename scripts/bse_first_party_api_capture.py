"""Capture and validate the first-party BSE API contracts discovered from live pages.

This is deliberately exchange-local.  It does not certify completeness by HTTP status
alone: it records request method, URL, POST payload, response status/body, JSON shape,
and row/date evidence.  Known BSE service names are classified but the live request
contract is taken from the browser rather than guessed.
"""
...
END=date.fromisoformat(os.getenv("TARGET_DATE") or str(date.today()))
LOOKBACK=int(os.getenv("LOOKBACK_DAYS") or "90")
START=END-timedelta(days=max(0,LOOKBACK-1))
OUT=Path("artifacts/bse_api_contract"); OUT.mkdir(parents=True,exist_ok=True)
...
PAGES={
 ...
 "rights":"https://www.bseindia.com/markets/publicissues/furtherissuesummary_ri",
 "preferential":"https://www.bseindia.com/markets/publicissues/furtherissuesummary_pref",
}
KNOWN={
 "BulkDeal_Beta":"bulk", "BlockDeal_Beta":"block", "getCorp_Regulation_ng":"insider",
 "Pubissues_FurtherIssuesummary_RI_isd_ng":"rights",
 "Pubissues_FurtherIssuesummary_Pref_isd_ng":"preferential",
 "Pubissues_FurtherXbrlview_pref_ng":"preferential",
}
o=Options()
...
def controls():
 return d.execute_script("return Array.from(document.querySelectorAll('input,select,button')).map(x=>({id:x.id||'',name:x.name||'',value:x.value||'',type:x.type||'',text:(x.innerText||'').trim(),cls:x.className||''})).filter(x=>x.id||x.name||x.value||x.text)")

def set_dates():
 nodes=d.find_elements("css selector","input[name='datepicker'],input[id*='datepicker' i],input[class*='datepicker' i]")
 if len(nodes)<2:
  nodes=[x for x in d.find_elements("css selector","input") if re.search(r"date|from|to",((x.get_attribute('id') or '')+' '+(x.get_attribute('name') or '')+' '+(x.get_attribute('class') or '')),re.I)]
 ...
 for n,v in zip(nodes[:2],(START.strftime('%d/%m/%Y'),END.strftime('%d/%m/%Y'))):
  try:
   n.click(); n.send_keys(Keys.CONTROL,'a'); n.send_keys(v); n.send_keys(Keys.TAB)
  except Exception: pass
 clicked=d.execute_script("const xs=Array.from(document.querySelectorAll('button,input[type=submit],input[type=button],a'));const n=xs.find(x=>/search|submit|show/i.test((x.innerText||x.value||'').trim())&&!/reset|clear/i.test((x.innerText||x.value||'').trim()));if(n){n.click();return true}return false")
 time.sleep(4)
 return {"status":"attempted","clicked_search":bool(clicked),"start":str(START),"end":str(END)}
  ...
  try:
   ...
   if method=='Network.requestWillBeSent':
    ...
    if 'api.bseindia.com' in u:
     x={'requestId':p.get('requestId'),'url':u,'status':r.get('status'),'mimeType':r.get('mimeType'),'headers':r.get('headers',{})}
     x.update(reqs.get(p.get('requestId'),{}))
     try:
      body=d.execute_cdp_cmd('Network.getResponseBody',{'requestId':p.get('requestId')}).get('body','')
      x['body_bytes']=len(body); x['body']=body[:1000000]
      try:
...
try:
 for ds,url in PAGES.items():
  ...
  before=controls(); hist=set_dates() if ds in ('bulk','block','insider') else {'status':'not_applicable'}
  time.sleep(2)
  events=capture_api()
  classified=[]
  for e in events:
...
summary={k:(v['api_request_count'],v['known_api_count'],v['historical_test'].get('status')) for k,v in report['datasets'].items()}
